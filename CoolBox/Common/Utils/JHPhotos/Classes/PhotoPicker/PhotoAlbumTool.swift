...
struct PhotoAlbumTool {
    
    // MARK: - public

    /// 获取Photo内所有相册
    static func getPhotoAlbumList() -> [PhotoListAlbum] {
        var photoAblums: [PhotoListAlbum] = []
        let smartAlbums = PHAssetCollection.fetchAssetCollections(with: .smartAlbum, subtype: .albumRegular, options: nil)
        smartAlbums.enumerateObjects({ (collection, idx, stop) in
            if !shouldFilterAblum(collection.localizedTitle) {
    ...
    /// - Returns: [PHAsset]
    static func getAllAssetInPhotoAblum(_ ascending: Bool) -> [PHAsset]? {
        var assets: [PHAsset] = []
    ...
    /// - Returns: [PHAsset]
    static func getAssets(in assetCollection: PHAssetCollection, ascending: Bool) -> [PHAsset] {
        var arr: [PHAsset] = []
        let result = self.fetchAssets(in: assetCollection, ascending: ascending)
        result.enumerateObjects({ (obj, idx, stop) in
            if obj.mediaType == .image || obj.mediaType == .video {
                arr.append(obj)
            }
        })
        return arr
    ...
    static func enumerateAssets(in assetCollection: PHAssetCollection, ascending: Bool, block: @escaping (PHAsset, Int, UnsafeMutablePointer<ObjCBool>) -> Swift.Void) {
        let result = self.fetchAssets(in: assetCollection, ascending: ascending)
        result.enumerateObjects(block)
    }
    
    /// 获取asset对应的图片
    static func requestImage(for asset:PHAsset, size: CGSize, resizeMode: PHImageRequestOptionsResizeMode, contentMode: PHImageContentMode, result: @escaping (_ image: UIImage?) -> Void) {
        /**
    ...
    /// 获取asset对应的imageData
    static func requestImageData(for asset:PHAsset, result: @escaping (_ imageData: Data?) -> Void) {
        /**
         resizeMode：对请求的图像怎样缩放。有三种选择：None，默认加载方式；Fast，尽快地提供接近或稍微大于要求的尺寸；Exact，精准提供要求的尺寸。
         deliveryMode：图像质量。有三种值：Opportunistic，在速度与质量中均衡；HighQualityFormat，不管花费多长时间，提供高质量图像；FastFormat，以最快速度提供好的质量。
         这个属性只有在 synchronous 为 true 时有效。
    ...
    // MARK: - private
    static func transformCNAblumTitle(_ title: String?) -> String? {
    ...
    static func shouldFilterAblum(_ title: String?) -> Bool {
        guard let title = title else { return true }
        // 过滤掉视频、最近删除、慢动作
        if (title == "Recently Deleted" || title == "最近删除")
            || (title == "Slo-mo" || title == "慢动作")
            || (title == "Videos" || title == "视频")
        { return true }
        return false
    }

    /// 获取assetCollection内所有的PHAsset
    static func fetchAssets(in assetCollection: PHAssetCollection, ascending: Bool) -> PHFetchResult<PHAsset> {
        let options = PHFetchOptions()
        options.sortDescriptors = [NSSortDescriptor(key: "creationDate", ascending: ascending)]
        return PHAsset.fetchAssets(in: assetCollection, options: options)
    }
    
    static func compressImageQuality(_ imageData: Data?, toKByte maxLengthKb: Int, result: ((_ data: Data?) -> Void)?) {
        let maxLength = maxLengthKb * 1000
        if let data = imageData {
            // 大小不超过 10%
            if data.count < Int(Double(maxLength) * 1.1) {
                result?(data)
                return
            }
            else {
    ...
    static func compressImageQuality(_ image: UIImage, toKByte maxLengthKb: Int, result: @escaping (_ image: UIImage?) -> Void) {
        let maxLength = maxLengthKb * 1000
    ...
    private static func compressObjImageQuality(_ image: UIImage, maxLength: Int, result: ((_ data: Data?) -> Void)?) {
