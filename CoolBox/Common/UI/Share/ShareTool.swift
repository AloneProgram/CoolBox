...
class ShareTool: NSObject, UIDocumentPickerDelegate {
    ...
    public func shareMore( content: ShareContent) {
     ...
     func shareLocal(content: ShareContent) {
         guard let taskUrl = URL(string:  content.shareUrl) else { return }
         ...
         session.downloadTask(with: request) { [weak self] tempUrl, response, error in
             ...
             do {
                 ...
                 DispatchQueue.main.async {
                     self.saveFileToPhone(url: destinationPath, content: content)
                 }
             } catch let error {
                 debugPrint(error)
                 EToast.showFailed("\(error.localizedDescription)")
             }
         }.resume()
     }
    ...
    func saveFileToPhone(url: URL, content: ShareContent) {
